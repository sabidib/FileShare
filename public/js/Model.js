...
m.isUserConnected = function(username, callback) {
    ...
    this.socket.emit('isUserConnected', {
        'username': username
    });
}
...
m.loginUser = function(username, callback) {
    ...
    this.setCallback('loginUserResponse', function(data) {
        callback(data);
        setTimeout(function() {
            md.binarySocket.send("file", {
                'username_get_socket': username
            })
        }, 400);
    });
...
m.getFilesFromUser = function(username, callback) {
    ...
    this.socket.emit('getFilesFromUser', {
        'username': username
    });
}
...
m.getStream = function(data, callback) {
    ...
    this.socket.emit('getStream', {
        'file_id': data.request.file_id,
...
m.createShareGroup = function(name, callback) {
    ...
    this.socket.emit('createShareGroup', {
        'name': name
    });
}
...
m.addUsersToShareGroup = function(usernames, shareGroupID, callback) {
    ...
    this.socket.emit('addUsersToShareGroup', {
        'usernames': usernames,
